...
export function UserAuthenticated() {
    ...
    return (
        <>
            {isAuthenticated && (
                <UserContext.Provider value={{ ...profile.payload, mailboxSettings: mailboxSettings.payload }}>
                    <p>
                        You are signed in! <CurrentProfileInfo></CurrentProfileInfo>
                    </p>
                    {mailboxSettings && !mailboxSettings.isLoading && (
                        <MailboxSettingsForm model={mailboxSettings.payload}></MailboxSettingsForm>
                    )}
